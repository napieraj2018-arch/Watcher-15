...
def exact_departure(driver, expected_dep):
    qs=parse_qs(urlparse(driver.current_url).query)
    selected=(qs.get('date') or [''])[0]
    return selected==expected_dep.isoformat()

def _card_for_total_label(driver, lab):
    # Return the NEAREST offer-sized ancestor, not the whole results container.
    return driver.execute_script(r"""
        let el = arguments[0];
        let fallback = null;
        for (let i = 0; i < 12 && el && el !== document.body; i++, el = el.parentElement) {
            const t = (el.innerText || '').replace(/\s+/g, ' ').trim();
            if (!/Cena całkowita\s*[0-9][0-9 .]*\s*zł/i.test(t)) continue;
            if (t.length < 60 || t.length > 3000) continue;
            if (!fallback) fallback = el;
            const hasAdult = /Dorosły\s*[0-9][0-9 .]*\s*zł/i.test(t);
            const hasChild = /(?:1\.|2\.)?\s*dziecko\s*[0-9][0-9 .]*\s*zł/i.test(t);
            if (hasAdult && hasChild) return el;
        }
        return fallback;
    """, lab)

def _nearest_href(driver, chosen):
    return driver.execute_script(r"""
        let el=arguments[0];
...
def parse_cards(driver):
    ...
    for lab in labels:
        try:
            ...
            if not mr: mr=re.search(r'(?:ocena|rating)\s*[: ]\s*([0-9](?:[.,][0-9])?)',text,re.I)
            ...
            href=_nearest_href(driver,chosen) or driver.current_url
            cards.append({'key':key,'total':total,'meal':meal,'stars':stars,'rating':rating,'reviews':reviews,'text':text,'href':href})
    ...
    try:
        ...
        for delta in cfg['depart_in_days']:
            ...
            if cards:
                ...
                print('SUNFUN_FAMILY_TOTAL_VERIFIED',dep,cards[0]['total'],len(cards));print('SUNFUN_CARD_SAMPLE',cards[0]['text'][:1800])
            print('SUNFUN_CARDS',dep,len(cards),[(x['total'],x['stars'],x['rating'],x['reviews'],x['meal'],x['href'][:100]) for x in cards[:12]])
            for x in cards:
        ...
        for dep,url,x in matches[:10]:
            ...
            confirm=next((y for y in fresh if y['total']==x['total'] and y['key']==x['key'] and qualifies(y,cfg)),None)
