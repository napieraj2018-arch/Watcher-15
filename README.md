...
- **Travelplanet** — natywna serializacja 2+2 została potwierdzona: wariant tablicowy zachowuje na serwerze `nl_occupancy_adults=2`, `nl_occupancy_children=2` oraz `nl_ages_children=[5,7]` w `data-search_params`. To usuwa wcześniejszy problem gubienia dzieci i kanał ma `exact_party=true`. W ścisłym oknie 24–26.09 serwer zwrócił jednak `boxesFound=0`, więc nie ma jeszcze dowodu żywej dostępności i końcowej ceny rodzinnej; kanał pozostaje `diagnostic`.
